...
async def list_functions(
    ...
    try:
        functions = await FunctionManager.list_functions(db)
...
async def create_function(
    ...
    try:
        result = await FunctionManager.create_function(db, request.function_sql)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def drop_function(
    ...
    try:
        result = await FunctionManager.drop_function(db, function_name)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
