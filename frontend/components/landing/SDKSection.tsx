...
const snippets: Record<Tab, { label: string; lines: { text: string; type: string }[] }> = {
  Database: {
    label: 'Query anything. No cap.',
    lines: [
      { text: "import { createClient } from '@zendbx/sdk'", type: 'import' },
      { text: '', type: '' },
      { text: 'const db = createClient(ZENDBX_URL, ZENDBX_KEY)', type: 'code' },
      { text: '', type: '' },
      { text: '// fetch active users — it just hits different', type: 'comment' },
  ...
  Auth: {
    label: 'Auth that slaps, zero setup.',
    lines: [
      { text: "import { createClient } from '@zendbx/sdk'", type: 'import' },
      { text: '', type: '' },
      { text: 'const db = createClient(ZENDBX_URL, ZENDBX_KEY)', type: 'code' },
      { text: '', type: '' },
      { text: '// sign up fr fr — one line', type: 'comment' },
      ...
      { text: '})', type: 'code' },
      { text: '', type: '' },
      { text: '// sign in — bussin', type: 'comment' },
      { text: 'await db.auth.signIn({ email, password })', type: 'code' },
    ],
  },
  Realtime: {
    label: 'Live updates. Main character energy.',
    lines: [
      { text: "import { createClient } from '@zendbx/sdk'", type: 'import' },
      { text: '', type: '' },
      { text: 'const db = createClient(ZENDBX_URL, ZENDBX_KEY)', type: 'code' },
      { text: '', type: '' },
      { text: '// subscribe to new messages — stay woke', type: 'comment' },
      { text: "const sub = db.realtime.from('messages')", type: 'code' },
      { text: "  .on('INSERT', (payload) => {", type: 'chain' },
      { text: "    console.log('new drop:', payload.new)", type: 'inner' },
      { text: '  })', type: 'chain' },
      { text: '  .subscribe()', type: 'chain' },
      { text: '', type: '' },
      { text: '// peace out when done', type: 'comment' },
      { text: 'sub.unsubscribe()', type: 'code' },
    ],
  },
  Storage: {
    label: 'File uploads. Slay.',
    lines: [
      { text: "import { createClient } from '@zendbx/sdk'", type: 'import' },
      { text: '', type: '' },
      { text: 'const db = createClient(ZENDBX_URL, ZENDBX_KEY)', type: 'code' },
      { text: '', type: '' },
      { text: "// upload an avatar — lowkey easy", type: 'comment' },
      { text: "await db.storage.from('avatars').upload(", type: 'code' },
      { text: "  'user-123.png',", type: 'prop' },
      { text: '  file,', type: 'prop' },
      { text: "  { contentType: 'image/png' }", type: 'prop' },
      { text: ')', type: 'code' },
      { text: '', type: '' },
      { text: '// get the public URL — sheesh', type: 'comment' },
      { text: "db.storage.from('avatars').getPublicUrl('user-123.png')", type: 'code' },
    ],
  },
};
