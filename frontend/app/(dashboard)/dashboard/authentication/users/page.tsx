...
export default function UsersPage() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      if (currentUserRole === 'admin') {
        ...
        const res = await fetch(`http://localhost:8000/api/admin/users?${params}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
  ...
  const confirmSuspend = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`http://localhost:8000/api/admin/users/${selectedUser.id}/suspend`, {
        method: 'PATCH',
        headers: {
  ...
  const handleUnsuspend = async (userId: string) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`http://localhost:8000/api/admin/users/${userId}/unsuspend`, {
        method: 'PATCH',
        headers: { 'Authorization': `Bearer ${token}` }
      });
